<template>
  ...
  <div v-bind:class="{'space-y-1': label}">
    <div v-show="label">
      <label :for="id" class="text-sm font-semibold dark:text-slate-300">{{ label }}</label>
    </div>
      <input :id="id" v-bind:value="modelValue" v-on:input="updateValue" v-bind="$attrs" v-bind:class="[borderColor]"
